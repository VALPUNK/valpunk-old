...
export default class Estimates extends React.Component<Props> {
  public calcTotalPrice = (percent: number) => {
    const discountedPackageTotal = this.props.rawTotal * ((100 - percent) / 100)
  ...
  public calcLumpSum = () => {
    const lumpSum =
      this.props.data.costPerPeriod * this.props.data.weeks.length * 0.5
    return lumpSum
  }
    ...
    return (
      <>
        <Center size={8}>
          ...
          <Point
            name={`3 Months at $${this.calcMonthlyPrice(
              10,
              3
            ).toLocaleString()}/month, totalling $${this.calcTotalPrice(
              10
            ).toLocaleString()}.`}
            id="1"
            subtask
          />
          <Point
            name={`6 Months at $${this.calcMonthlyPrice(
              20,
              6
            ).toLocaleString()}/month, totalling $${this.calcTotalPrice(
              20
          ...
          />
          <Point
            name={`9 Months at $${this.calcMonthlyPrice(
              10,
              9
            ).toLocaleString()}/month, totalling $${this.calcTotalPrice(
              10
          ...
          <Point
            name={`At $${this.props.data.costPerPeriod}/${
              this.props.data.devPeriod
            } for the described set of tasks.`}
            subtask
          />
          <Point name="Lump Sum" />
          <Point
